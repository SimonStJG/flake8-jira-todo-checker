...
### disallowed-jira-statuses and disallowed-jira-resolutions

If a TODO is attached to a JIRA card whose status is in `disallowed-jira-statuses` then an error will be reported, ditto
if the JIRA card has a resolution in `disallowed-jira-resolutions`.
